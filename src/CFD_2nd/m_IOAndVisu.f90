...
contains
  ...
  subroutine InitVisu(ChannelPrm)
    ...
    if(myistat/=0 .and. nrank==0) call MainLog%CheckForError(ErrT_Abort,"InitVisu", "Cannot openfile: "//trim(ChannelPrm))
    ...
    if(myistat/=0 .and. nrank==0) call MainLog%CheckForError(ErrT_Abort,"InitVisu","Cannot open file: "//trim(XdmfFile))
    ! XDMF/XMF Title
