...
contains
...
#ifndef CFDACM
    allocate(SumStat(35,nyp),Stat=ierror)
#else
    allocate(SumStat(10,nyc),Stat=ierror)
#endif
  ...
  subroutine clcStat_CH(ux,uy,uz,pressure)
    ...
    ! locals
    character(len=50)::filename
    integer::ic,jc,kc,im,jm,km,ip,jp,kp,myistat,ierror,nUnit
    ...
    if(nrank==0) then
      ...
      ELSE
        ...
        write(nUnit,'(A)')'  '
        do jc=1,nyp
          write(nUnit,'(40ES24.15)')SumStatR(1:35,jc)*infstime
        enddo
      ENDIF
      close(nUnit,IOSTAT=myistat)
    endif
  ...
  subroutine clcStat_CH(ux,uy,uz,pressure)
    ...
    integer:: ic,jc,kc,myistat,ierror,sum_indicator,nUnit
    ...
    if(nrank==0) then
      ...
      ELSE
        ...
        write(nUnit,'(A)')
        write(nUnit,'(A)')'yc, VolumeFrac, u, v, w, uu, vv, ww, uv, pr pr2'
        do jc=1,nyc
          isum_indicator= one/sumStatR(10,jc)
          write(nUnit,'(15ES24.15)')yc(jc),sumStatR(10,jc)*infstime *inxz,  &  ! 1,2
