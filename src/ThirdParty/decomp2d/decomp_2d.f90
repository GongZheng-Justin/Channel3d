...
module decomp_2d
  ...
  TYPE, public :: DECOMP_INFO
     ...
     integer, allocatable, dimension(:) ::             & ! Add this line by Gong Zheng, transpose_x_z purpose
          x2dist, z1dist, x1st, y1st, y2st, z2st,x2st, & ! Add this line by Gong Zheng, transpose_x_z purpose
          z1st, x1en, y1en,y2en, z2en,x2en, z1en         ! Add this line by Gong Zheng, transpose_x_z purpose     
     ...
     integer, allocatable, dimension(:) :: &    ! Add this line by Gong Zheng, transpose_x_z purpose
          x2cnts, z1cnts, x2disp, z1disp        ! Add this line by Gong Zheng, transpose_x_z purpose     
     ...
     integer :: x2count, z1count                !  Add this line by Gong Zheng, transpose_x_z purpose     
...
! use default integer for MPI_Alltoallw call
  ...
  TYPE(DECOMP_INFO), save, public :: decomp_main           ! Make it public by Gong Zheng.
  !TYPE(DECOMP_INFO), save, public :: phG,ph1,ph2,ph3,ph4  ! Delete this line by Gong Zheng.
  ...
  complex(mytype), allocatable, dimension(:) :: work1_c, work2_c  ! no need to do complex transpose, Gong Zheng
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  public :: decomp_2d_init, decomp_2d_finalize, &
       ...
       transpose_x_to_z, transpose_z_to_x, &  !  Add this line by Gong Zheng, transpose_x_z purpose
#ifdef OCC
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine decomp_2d_finalize
    ...
    decomp_buf_size = 0
    deallocate(work1_r,work2_r) ! change this line, original line: deallocate(work1_r,work2_r,work1_c,work2_c)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine decomp_info_init(nx,ny,nz,decomp) 
    ...
    allocate(decomp%x2dist(0:dims(2)-1),decomp%z1dist(0:dims(1)-1))    
    ...
    !============  Add following lines by Gong Zheng, transpose_x_z purpose ==========       
    allocate(decomp%z1cnts(0:dims(1)-1),decomp%x2cnts(0:dims(2)-1))
    allocate(decomp%x2disp(0:dims(2)-1),decomp%z1disp(0:dims(1)-1))
    ...
    if (buf_size > decomp_buf_size) then
       ...
       !if (allocated(work1_c)) deallocate(work1_c) ! delete this ling by Gong Zheng, no need to do complex transpose
       !if (allocated(work2_c)) deallocate(work2_c) ! delete this ling by Gong Zheng, no need to do complex transpose
       allocate(work1_r(buf_size), STAT=status)
       allocate(work2_r(buf_size), STAT=status)
       !allocate(work1_c(buf_size), STAT=status) ! delete this ling by Gong Zheng, no need to do complex transpose
       !allocate(work2_c(buf_size), STAT=status) ! delete this ling by Gong Zheng, no need to do complex transpose
       if (status /= 0) then
          errorcode = 2
          call decomp_2d_abort(errorcode, &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine decomp_info_finalize(decomp)
    ...
    implicit none
    integer :: i,ierror   ! add this line by gongzheng
    TYPE(DECOMP_INFO), intent(INOUT) :: decomp
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine get_dist(nx,ny,nz,decomp)
    ...
    ! === change the following line by Gong Zheng, transpose_x_z purpose ===
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! change this aubroutine by Gong Zheng, transpose_x_z purpose  
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !   Add "transpose_x_to_z.inc","transpose_z_to_x.inc" by Gong Zheng  
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
