...
contains
  ...
  subroutine InitStatVar_TG()
    ...
    if(nrank/=0) return
    open(69, file='./CFD/Results/TG_dissp.txt',status='replace',form='formatted',IOSTAT=myistat)
    if(myistat /= 0) call MainLog%CheckForError(ErrT_Abort,"InitStatVar_TG: ","Cannot open file")
    close(69)
  ...
  subroutine clcStat_TG(ux,uy,uz,pressure)
    ...
    if(nrank==0) then
      open(unit=69, file='./CFD/Results/TG_dissp.txt', status='old',position='append',form='formatted',IOSTAT=ierror )
      IF(ierror/=0) THEN
