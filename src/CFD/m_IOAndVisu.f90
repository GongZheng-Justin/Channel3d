...
contains
  ...
  subroutine InitVisu(ChannelPrm)
    ...
    integer::nUnitFile,myistat,nflds,ifld,iprec,i,j,k,nxOut,nyOut,nzOut
    ...
    if(myistat /= 0) then
      call MainLog%CheckForError(ErrT_Abort,"InitVisu: ", "Cannot openfile: "//trim(adjustl(ChannelPrm)) )
    endif
    read(nUnitFile, nml=IO_Options)
    close(nUnitFile)
    if(nrank==0)write(d_LogInfo_unit, nml=IO_Options)
    ...
    if(myistat /= 0) then
      call MainLog%CheckForError(ErrT_Abort,"InitVisu","Cannot open file:  "//trim(adjustl(XdmfFile)))
    endif
    ! XDMF/XMF Title
  ...
  subroutine write_restart(ntime,ux,uy,uz,pressure,HistXOld,HistYOld,HistZOld)
    ...
    call decomp_2d_write_var(fh,disp,y_pencil,      ux(ystart(1):yend(1),ystart(2):yend(2),ystart(3):yend(3)))
  ...
  subroutine read_restart(ux,uy,uz,pressure,HistXOld,HistYOld,HistZOld)
    ...
    character(24)::ch
    character(256)::chFile
    integer::fh,code,ntime,type_byte
    ...
    if(code /= 0 .and. nrank==0) then
      call MainLog%CheckForError(ErrT_Abort,"Read_Restart","Cannot open Restart file")
    endif

    call MPI_TYPE_SIZE(real_type,type_byte,code)
    if(ischeme==FI_AB2 .and. ReadHistOld) then
      byte_total=int(type_byte,8)*int(nxc,8)*int(nyc,8)*int(nzc,8)*7_MPI_OFFSET_KIND
    else
      byte_total=int(type_byte,8)*int(nxc,8)*int(nyc,8)*int(nzc,8)*4_MPI_OFFSET_KIND
    endif
    ...
    disp = 0_MPI_OFFSET_KIND
    call decomp_2d_read_var(fh,disp,y_pencil,      ux(ystart(1):yend(1),ystart(2):yend(2),ystart(3):yend(3)))
