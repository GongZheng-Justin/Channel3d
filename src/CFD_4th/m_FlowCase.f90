  ...
  subroutine InitStatVar()
    ...
    if(nrank==0) then
      ...
      if(IsUxConst)then
        ...
        if(iErr/=0 .and. nrank==0) call MainLog%CheckForError(ErrT_Abort,"InitCAStatistics","Cannot open file: "//trim(filename))
    ...
    if(clc_Spectra) then
      ...
      if(njSpecX>0) then
        ...
        if(nrankX==0) then
          ...
          open(70,file=filename,status='replace',form='formatted',IOSTAT=myistat)
          IF(myistat/=0) THEN
      ...
      if(njSpecZ>0) then
        ...
        if(nrankZ==0) then
          ...
          open(71,file=filename,status='replace',form='formatted',IOSTAT=myistat)
          IF(myistat/=0) THEN
            call MainLog%CheckForError(ErrT_Abort,"clcStat_CH","Cannot open file: "//trim(filename))
          ELSE
