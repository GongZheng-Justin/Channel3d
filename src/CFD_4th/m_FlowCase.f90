...
contains
  ...
  subroutine clcStat(ux,uy,uz,pressure)
    ...
    if(nrank==0 .and. IsUxConst) then
      open(79,file=trim(Res_Dir)//'PrGrad.txt',status='old',position='append',form='formatted',IOSTAT=myistat)
      if(myistat /= 0) then
        call MainLog%CheckForError(ErrT_Pass,"clcStat","Cannot open file 1")
      else
    ...
    if(clc_Spectra) then
      ...
      if(njSpecX>0) then
        ...
        if(nrankX==0) then
          ...
          write(filename,"(A,I10.10,A,I4.4)") trim(Res_Dir)//'SpecX',itime,'_',coord1
      ...
      if(njSpecZ>0) then
        ...
        if(nrankZ==0) then
          ...
          write(filename,"(A,I10.10,A,I4.4)") trim(Res_Dir)//'SpecZ',itime,'_',coord2
