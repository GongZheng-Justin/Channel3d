...
#ifdef CFDLPT_TwoWay
  use m_Variables,only: mb1,hi1,FpForce_x,FpForce_y,FpForce_z
#else
...
contains    
  ...
  subroutine clcRhsXYZ(ux,uy,uz,scalar,RhsX,RhsY,RhsZ,RhsC,HistXOld,HistYOld,HistZOld,HistCOld,pressure)
    ...
    real(RK),dimension(ystart(1):yend(1),ystart(2):yend(2),ystart(3):yend(3)),intent(inout)::HistXOld,HistYOld,HistZOld,HistCOld
    ! lcoals
...
#ifdef CFDLPT_TwoWay  
#ifdef ForceInCell
          convEd2= -h21-h22-h23 +xnu*dcq13+ gravity(2)+half*(FpForce_y(ic,jc,kc)+FpForce_y(ic,jm,kc))
#else
