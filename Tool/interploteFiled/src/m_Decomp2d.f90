module m_Decomp2d
  ...
  integer,save,public:: int_type,real_type
  integer,save,public:: int_byte,real_byte

  ! Define neighboring blocks (including the Bc info)
  !   second dimension 4 neighbour numProcessors
  integer,save,public,dimension(4):: myProcNghBC
  ...
  integer,dimension(3),save,public:: ystart,yend,ysize
...
contains
  ...
  subroutine decomp2d_init(nxc,nyc,nzc,row,col,BcOpt)
    ...
    ! Fourthly, MPI_TYPE and MPI_BYTE
  ...
  subroutine decomp2d_alloc(var,mb,opt_decomp,opt_global)
    ...
    real(RK), allocatable, dimension(:,:,:),intent(OUT) :: var
    ...
    if(alloc_stat/=0) call MainLog%CheckForError(ErrT_Abort,"decomp2d_alloc","Allocation failed")
  ...
  subroutine decomp2d_updateHalo(mat,mb,hi)
    ...
    real(RK), dimension(mb%xmm:mb%xpm,mb%ymm:mb%ypm,mb%zmm:mb%zpm),intent(INOUT):: mat
    ...
    if(mb%xme<hi%xmh .or. mb%yme<hi%ymh .or. mb%zme<hi%zmh .or.  &
       ...
       call MainLog%CheckForError(ErrT_Abort,"decomp2d_updateHalo","HaloSize error")
    endif
  ...
  subroutine decomp2d_readVar(fh,disp,var,opt_decomp)
    ...
    disp = disp + int(sizes(1),8)*int(sizes(2),8)*int(sizes(3),8)*int(real_byte,8)
  ...
  subroutine decomp2d_writeVar(fh,disp,var,opt_decomp)
    ...
    disp = disp + int(sizes(1),8)*int(sizes(2),8)*int(sizes(3),8)*int(real_byte,8) ! modified by gongzheng, 2012-02-18
