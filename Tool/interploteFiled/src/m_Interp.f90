...
contains
  ...
  subroutine Interp_uvwp(mbOld,ArrOld)
    ...
    real(RK)::prx1,pry1,prz1,prx2,pry2,prz2,umean,umeanR

    ! open "RestartOld" and "RestartNew"
    chFileOld = OldRestartName;  chFileNew = NewRestartName; 
    ...
    if(ierror /= 0 .and. nrank==0) then
      call MainLog%CheckForError(ErrT_Abort,"Read_Restart","Cannot open RestartOld")
    endif
    ...
    if(IsUxConst) then
      ...
      do kc=decomp_uxNew%yst(3),decomp_uxNew%yen(3)
        do jc=decomp_uxNew%yst(2),decomp_uxNew%yen(2)
          do ic=decomp_uxNew%yst(1),decomp_uxNew%yen(1)
            umean=umean+uxNew(ic,jc,kc)*(ypNew(jc+1)-ypNew(jc))
          enddo
        enddo
      enddo
      umean=umean/(real(nxcNew,RK))/(real(nzcNew,RK))/yly
      call MPI_ALLREDUCE(umean,umeanR,1,real_type,MPI_SUM,MPI_COMM_WORLD,ierror)
      uxNew= (ubulk/umeanR)*uxNew
    endif
