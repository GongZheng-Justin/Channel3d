module m_Parameters
  ...
  character(64),public,save:: OldMeshName  
  character(64),public,save:: NewMeshName
...
contains
  ...
  subroutine ReadAndInitParameters(chFile)
    ...
    NAMELIST/BasicParam/nxpOld,nypOld,nzpOld,nxpNew,nypNew,nzpNew,BcOption,uxBcValue,uyBcValue,uzBcValue, &
                        ...
                        ubulk,OldMeshName,NewMeshName,OldRestartName,NewRestartName

    nUnitFile = d_read_Param_unit 
    open(unit=nUnitFile, file=chFile, status='old',form='formatted',IOSTAT=myistat )
    if(myistat/=0) call MainLog%CheckForError(ErrT_Abort,"ReadAndInitParameters","Open file wrong")
